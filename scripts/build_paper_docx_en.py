# -*- coding: utf-8 -*-
"""Build paper_draft_en.docx from docs/paper_draft_en.md.

English version: Calibri body/headings (no CJK fonts needed). US Letter,
1in margins, 9360 DXA content width. IEEE TIM double-column compatible.
"""
...
OUT = ROOT / "docs" / "paper_draft_en.docx"

# ---------------------------------------------------------------- tokens ----
BODY_LATIN = "Calibri"
BODY_EAST = "Calibri"          # no CJK needed, but keep for safety
HEAD_LATIN = "Calibri"
HEAD_EAST = "Calibri"
CODE_LATIN = "Consolas"
...
def add_figure(doc, alt: str, rel: str):
    """Insert a centered picture with a 9pt Calibri bold caption."""
...
def build():
    ...
    hr = header_p.add_run("Coverage-Aware Sampling and Lightweight Hierarchical PD Detection · Manuscript Revision")
    ...
    while i < n:
        ...
        if stripped.startswith("|"):
            ...
            if rows:
                add_table(doc, rows, CAPTIONS_EN[table_idx])
                table_idx += 1
            prev_num = 0
            cur_decimal_id = None
            continue
    ...
    props.title = "Coverage-Aware Sampling and Lightweight Hierarchical Weakly-Supervised Detection for Extremely Long Sparse Three-Phase Partial Discharge Signals"
    props.author = "(Authors pending)"
    props.subject = "VSB Partial Discharge Detection · Manuscript Revision"
