# -*- coding: utf-8 -*-
"""Robustness evaluation of the final 80k mainline (cnn/attention/mean).

Mirror of scripts/run_robustness.py, but for the paper's final lightweight
model (80,113 params) instead of the 203k reference.  Uses the 5 dev-fold
checkpoints trained for the ensemble blind evaluation
(results/ablations/dev_k8_blind_ckpts/enc_cnn__mil_attention__ph_mean) on the
same StratifiedGroupKFold(5, seed=42) development splits of the paper's locked
features_policy_mixed_k8 cache.  Perturbations are applied only at inference
time; models are never retrained on perturbed data.

Tests (same as the reference report):
  1. additive Gaussian noise (SNR 20/10/5 dB)
  2. amplitude scaling (0.8x / 1.2x)
  3. time shift (+-64 / +-128 samples, zero-padded, features recomputed)
...
NOTE: the mean phase-interaction broadcasts the same vector to all three
phases, so phase-level probabilities are identical across phases by design;
phase-level metrics are a measurement-shared score ranked against phase
labels, not per-phase discrimination.

Usage:
    python scripts/run_robustness_80k.py
"""
...
def main() -> None:
    ...
    ap.add_argument(
        "--ckpts",
        default="results/ablations/dev_k8_blind_ckpts/enc_cnn__mil_attention__ph_mean",
    )
    ap.add_argument("--out", default="results/robustness_report_80k.json")
    ...
    report: dict = {
        ...
        "phase_metric_note": (
            "mean phase-interaction broadcasts the same vector to all three "
            "phases; phase-level metrics use a measurement-shared score ranked "
            "against phase labels, not per-phase discrimination"
        ),
    }
    ...
    report["shift_metadata"] = {
        ...
        "note": ("same zero-pad protocol as the 203k reference robustness "
                 "report; the old cyclic-roll artifact was already replaced"),
    }
    sr_hz = 40_000_000
